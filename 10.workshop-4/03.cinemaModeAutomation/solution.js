...
function setLightBirghtness(brightness) {
  Shelly.call("RGB.Set", {id: 0, on: true, brightness: brightness});
  for (url of DIMMER_URLS) {
...
lightControl.on("single_push", function(e){
  setLightBirghtness(MOVIE_BRIGHTNESS);
});

lightControl.on("double_push", function(e){
  setLightBirghtness(NO_MOVIE_BRIGHTNESS);
});